...
function App() {
  ...
  const usage = Math.min((requests / RATE_LIMIT) * 100, 100);
  const zone = isRateLimited || requests >= RATE_LIMIT ? 'blocked' : usage >= 80 ? 'warning' : 'safe';
  const zoneStyles = {
  ...
  return (
    <div className="min-h-screen bg-stone-100 text-stone-800">
      <main className="mx-auto max-w-5xl px-5 py-8 sm:px-8 sm:py-12">
        ...
        <section className="py-12 sm:py-16"><p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-stone-500">Rate limiter</p><div className="flex flex-col justify-between gap-7 sm:flex-row sm:items-end"><div><h1 className="max-w-xl text-4xl font-semibold tracking-tight text-stone-900 sm:text-5xl">A clearer view of your request window.</h1><p className="mt-4 max-w-lg text-base leading-7 text-stone-600">Send a request to your local endpoint and see where it sits within the configured limit.</p></div><div className={`inline-flex shrink-0 items-center gap-2 self-start rounded-full border px-3 py-1.5 text-xs font-medium sm:self-auto ${activeZone.badge}`}><span className={`h-1.5 w-1.5 rounded-full ${activeZone.dot}`} />{activeZone.label}</div></div></section>
